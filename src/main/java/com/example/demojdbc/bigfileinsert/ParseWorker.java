package com.example.demojdbc.bigfileinsert;
...
public class ParseWorker implements Runnable{
	 ...
	 * @param barrier
	 */
	public ParseWorker(RandomAccessFile fileAccess,FilePartition filePartition,CyclicBarrier barrier) {
	...
	/**
	 * 解析逻辑:1、将文件切分区域内的数据映射到内存镜像中
	 *        2、读取内存镜像中的数据到缓冲区中，以便进行数据处理
	 *        3、循环处理缓冲区中的数据，以 \r\n 为一行数据，交由DataHandler进行解析
	 *        4、切分区域内的数据读取完成，则通知栅栏
	 */
	public void run() {
		try {

			long start = System.currentTimeMillis();
			MappedByteBuffer mapBuffer = fileAccess.getChannel().map(MapMode.READ_ONLY,filePartition.getStart(), this.sliceSize);
